...
test "reader and command overlay shortcuts do not overlap" {
    ...
    try std.testing.expect(!commandOverlayShortcut(c.SDLK_R, c.SDL_KMOD_GUI));
    try std.testing.expect(!commandOverlayShortcut(c.SDLK_T, c.SDL_KMOD_GUI | c.SDL_KMOD_SHIFT));
