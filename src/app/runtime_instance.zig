...
pub fn releaseTerminalEntrySyncPolicyAfterMutation(policy: *TerminalEntrySyncPolicy, terminal_entries_mutated: bool) void {
    if (terminal_entries_mutated) {
        policy.* = .normal;
    }
}
...
test "releaseTerminalEntrySyncPolicyAfterMutation resumes normal sync" {
    var policy: TerminalEntrySyncPolicy = .preserve_existing;
    releaseTerminalEntrySyncPolicyAfterMutation(&policy, false);
    try std.testing.expectEqual(TerminalEntrySyncPolicy.preserve_existing, policy);

    releaseTerminalEntrySyncPolicyAfterMutation(&policy, true);
    try std.testing.expectEqual(TerminalEntrySyncPolicy.normal, policy);
}
