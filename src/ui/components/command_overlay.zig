    ...
    fn handleEventFn(self_ptr: *anyopaque, host: *const types.UiHost, event: *const c.SDL_Event, actions: *types.UiActionQueue) bool {
        ...
        switch (event.type) {
            c.SDL_EVENT_KEY_DOWN => {
                ...
                const has_shortcut_mod = (mod & (c.SDL_KMOD_GUI | c.SDL_KMOD_SHIFT | c.SDL_KMOD_CTRL | c.SDL_KMOD_ALT)) != 0;
                if (key == c.SDLK_ESCAPE and !has_shortcut_mod) {
                    actions.append(.ToggleCommandOverlay) catch |err| {
...
test "escape closes remote terminal overlay instead of sending terminal input" {
    ...
    var lock_event = keyDownEvent(c.SDLK_ESCAPE, c.SDL_KMOD_CAPS | c.SDL_KMOD_NUM);
    try std.testing.expect(ui_component.vtable.handleEvent.?(ui_component.ptr, &host, &lock_event, &actions));
    const lock_action = actions.pop() orelse return error.TestExpectedNonNull;
    switch (lock_action) {
        .ToggleCommandOverlay => {},
        else => return error.TestUnexpectedResult,
    }
