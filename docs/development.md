...
The `publish-apps` target is branch-aware. From `main`, it rebuilds and replaces only `/Applications/Architect (Stable).app`; from `scratch`, it rebuilds and replaces only `/Applications/Architect (Scratch).app`. It clears quarantine attributes, registers the refreshed app with Launch Services, removes stale Architect Dock tiles, adds the Stable/Scratch Dock icons, and restarts the Dock. Never build Stable from `scratch` or Scratch from `main`. The `stable` and `scratch` targets launch the installed `/Applications` bundles, start a fresh named session when `SESSION` is unset, and restore that session when `SESSION` is set. The explicit `stable-new`, `scratch-new`, `stable-restore`, and `scratch-restore` targets are available for scripts that should avoid that conditional behavior.
