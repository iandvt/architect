...
| `ui/components/session_picker_overlay.zig` | Expanding saved-session picker opened with Cmd+Shift+S. It lists saved sessions for the active channel, supports search, keyboard selection, and click selection, and emits `OpenNamedSession` so runtime can launch the selected session in a new window. | `SessionPickerOverlayComponent`, `addSession()` | `ui/components/expanding_overlay`, `ui/components/search_utils`, `ui/components/flowing_line`, `ui/first_frame_guard`, `geom`, `gfx/primitives`, `c` |
