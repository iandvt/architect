...
PTY + ghostty-vt terminal state
    | UI component captures input and forwards key/text/paste/scroll actions
    v
ui/components/command_overlay.zig renders a fullscreen frame
    | render/renderer.renderSessionIntoRect()
    v
Persistent remote terminal overlay (one per grid slot, compacted with grid slots, not persisted, not a grid slot)
```

### Saved Session Picker Path

```
Cmd+Shift+S
    | ui/components/session_picker_overlay.zig filters saved sessions
    v
UiAction.OpenNamedSession
    | app/runtime.zig handles action
    v
app/runtime_instance.launchSessionInNewWindow()
    | open -n <bundle> --args --instance <channel> --session <id>
    v
New Architect window restored from instances/<channel>/<session>/
```
...
| `app/runtime_instance.zig` | Per-instance runtime glue kept out of the frame loop: run options, window title construction, saved-session listing and relaunch, startup restore entries, manual resume prefill, and terminal persistence synchronization | `RunOptions`, `windowTitleForSession()`, `listSavedSessionsForChannel()`, `launchSessionInNewWindow()`, `restoredTerminalEntriesForStartup()`, `seedSessionAgentMetadataFromEntry()`, `prefillManualResumeCommandFromEntry()`, `syncPersistenceTerminalEntriesFromSessions()` | `config`, `session/state`, `app/terminal_history`, `toml` |
...
| `ui/components/session_picker_overlay.zig` | Expanding saved-session picker opened with Cmd+Shift+S. It lists saved sessions for the active channel, shows the current session as disabled, supports search, keyboard selection, and click selection, and emits `OpenNamedSession` so runtime can launch a selected non-current session in a new window. | `SessionPickerOverlayComponent`, `addSession()` | `ui/components/expanding_overlay`, `ui/components/search_utils`, `ui/components/flowing_line`, `ui/first_frame_guard`, `geom`, `gfx/primitives`, `c` |
| `ui/components/command_overlay.zig` | Fullscreen persistent terminal overlay for one-off commands, opened with Cmd+T, rendered above the grid/full scene without consuming a grid slot. Escape and the close button hide the overlay while preserving the selected slot's helper shell; wheel input scrolls the overlay's terminal scrollback. | `CommandOverlayComponent`, `setOpen()` | `app/command_overlay`, `ui/components/fullscreen_overlay`, `ui/components/search_utils`, `input/mapper`, `app/input_keys`, `font_cache`, `c` |
| `app/command_overlay.zig` | Runtime state for the remote terminal overlay's PTY-backed sessions. Owns one `CommandOverlay` per grid slot, with each overlay holding its own `SessionState`, terminal sizing, input forwarding, scroll handling, output draining, and renderer cache entry; hiding an overlay keeps that slot's shell and scrollback alive. Overlay sessions move with their grid slots during compaction, and foreground helper processes are included in quit safety checks and macOS agent teardown. | `CommandOverlaySet`, `CommandOverlay`, `panelRect()`, `terminalRect()`, `terminalSizeForRect()` | `session/state`, `render/renderer`, `app/layout`, `input/mapper`, `font`, `colors`, `geom`, `xev`, `c` |
| `ui/components/*` | Individual overlay and widget implementations conforming to `UiComponent` vtable. Includes: help overlay, worktree picker, recent folders picker (with instant search filtering), saved-session picker, diff viewer (with inline review comments), story viewer (PR story file visualization with rich markdown, anchor badges, bezier arrows, clickable links, and Cmd+F search — uses shared markdown parser/renderer pipeline and shared search utilities), reader mode overlay (uses shared search utilities), remote terminal overlay, fullscreen overlay helper (shared animation/scroll/close logic embedded by story, diff, reader, and remote terminal overlays), reusable aqua-style scrollbar widget, session interaction, toast, quit confirm, quit-blocking overlay, restart buttons, escape hold indicator, metrics overlay, global shortcuts, pill group, expanding overlay helper, button, confirm dialog, marquee label, hotkey indicator, flowing line, hold gesture detector. | Each component implements the `VTable` interface; overlays toggle via keyboard shortcuts or external commands and emit `UiAction` values. | `ui/component`, `ui/types`, `anim/easing`, `font`, `metrics`, `url_matcher`, `ui/session_view_state` |
